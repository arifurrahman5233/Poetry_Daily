...
const OrderList = () => {
    ...
    useEffect(() => {
        const url = `https://murmuring-scrubland-25292.herokuapp.com/user/order?userEmail=${user.email}`
    ...
    const handleDeleteOrder = id => {
        ...
        if (proceed) {
            const url = `https://murmuring-scrubland-25292.herokuapp.com/order?userEmail=${user.email}`;
            fetch(url, {
                method: 'DELETE'
            })
                .then(res => res.json())
